...
//
// Deliberately a twin of the ssh package's kit: a way in is a way in, and the
// two are not abstracted over (see the proposal — the duplication is the design).
// What travels is the consumer's build output, read here and named inside the kit;
// how it is written on the far side is the core's script.
...
export async function podmanKit(spec: KitSpec): Promise<Kit> {
  ...
  if (spec.relay) {
    if (!spec.gateway) {
      throw new PodmanSpecError("a relayed kit needs a gateway bundle — nothing to relay to");
    }
