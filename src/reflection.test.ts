...
declare module "./hooks" {
  interface ActorReflection {
    "testPlugin.getCount": () => Promise<number>;
    "testPlugin.getLog": () => Promise<Logger>;
    "plug.ping": () => Promise<string>;
    "plug.pong": () => Promise<string>;
  }
}
...
const testPlugin: ActorPlugin = async (config) => {
  return mergeConfigs(config, {
    $reflectionMethods: {
      "testPlugin.getCount": async function () {
        const s = this.state as InnerState;
        return s.count;
      },
      "testPlugin.getLog": async function () {
...
describe("actor reflection", () => {
  ...
  it("plugin registers reflection via mergeConfigs", async () => {
    ...
    const count: number = await proc.$reflection["testPlugin.getCount"]();
    expect(count).toBe(42);
  });
  ...
  it("plugin reflection works with async setup and multiple plugins", async () => {
    ...
    const log: Logger = await proc.$reflection["testPlugin.getLog"]();
  ...
  it("one plugin registers two methods", async () => {
    ...
    expect(await refl["plug.ping"]()).toBe("a");
    expect(await refl["plug.pong"]()).toBe("b");
  });
  ...
  it("two plugins: debugLogger + testPlugin, sync setup", async () => {
    ...
    expect(await refl["testPlugin.getCount"]()).toBe(0);
  });
  ...
  it("plugin-registered reflection does not clobber across spawns", async () => {
    ...
    const c1: number = await proc1.$reflection["testPlugin.getCount"]();
    const c2: number = await proc2.$reflection["testPlugin.getCount"]();
    expect(c1).toBe(10);
    expect(c2).toBe(20);
  });
});
